...
- ENHANCEMENTS (this build):
    ...
    - ✨ Library: CSS grid with rating-only badges (exact DOM as sample)
"""
...
# --- Library grid styles (inject CSS) ---
st.markdown("""
<style>
:root{
...
def get_book_metadata(isbn: str) -> dict:
    ...
    # Ratings: Google + OpenLibrary + placeholder
    ratings_parts = []
...
def render_library_grid(df: pd.DataFrame):
    """
    Render Library as an HTML/CSS card grid that matches the sample:
...
# ==== Cross-check helpers (normalizers) =======================================
