...
with tab1:
    ...
    if f:
        ...
        if isbn_scanned:
            st.success(f'ISBN: {isbn_scanned}')
            st.image(img,caption=isbn_scanned,width=160)
        else: st.error('No barcode detected')
with tab2:
...
if isbn_input:
    if isbn_input in library_df['ISBN'].values:
        ...
        if meta.get('Title'):
            ...
            if meta.get('Thumbnail','').startswith('http'): c1.image(meta['Thumbnail'],width=120)
            with c2:
            ...
            with b1:
                if st.button('➕ Add to Library'):
                    st.session_state['library']=pd.concat([library_df,pd.DataFrame([{'ISBN':isbn_input,**meta}])],ignore_index=True)
                    sync_session('library')
                    st.experimental_rerun()
            with b2:
                if st.button('⭐ Add to Wishlist'):
                    st.session_state['wishlist']=pd.concat([wishlist_df,pd.DataFrame([{'ISBN':isbn_input,**meta}])],ignore_index=True)
